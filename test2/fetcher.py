...
class DataFetcher:
    ...
    def fetch_index_daily(
        ...
        try:
            ...
            logger.error(f"fetch_index_daily {symbol} 失败: {e}")
            return None
    ...
    def fetch_index_valuation(self) -> Optional[pd.DataFrame]:
        """
        抓取主要指数 PE/PB 当前值及近10年分位
        数据源：AKShare index_value_name_funddb
        """
        try:
            _sleep()
            ...
            logger.error(f"fetch_index_valuation 失败: {e}")
            return None
    ...
    def fetch_index_pe_history(self, index_name: str, lookback_years: int = 10) -> Optional[pd.DataFrame]:
        ...
        try:
            _sleep()
            # AKShare 提供中证/沪深主要指数估值历史
            df = ak.index_value_hist_funddb(name=index_name)
            if df is None or df.empty:
                return None
            df.columns = [c.strip() for c in df.columns]
            if "日期" in df.columns:
    ...
    def _fetch_etf_flow_fallback(self, fund_code: str, lookback_days: int = 60) -> Optional[pd.DataFrame]:
        ...
        try:
            ...
            logger.error(f"_fetch_etf_flow_fallback {fund_code} 失败: {e}")
            return None
